...
uint32 garbage_collect_first(struct _nftl_zone* zone,_phy_block_info* block,uint16 page_num)
{
    ...
    //First calculate whether the number of valid pages is consistent with the value maintained in memory
    ...
    for(i=0; i<(zone->nand_chip->pages_per_blk-1); i++)
    {
        ...
        if(logic_page != 0xffffffff)
        {
            ...
            {
                //Effective page, need to be moved
...
//    _phy_block_info *p;
    ...
    {
        return NFTL_FAILURE;    /* The total amount of data written is small */
    }

    max_ec_blk = get_block_max_erase_time_from_free_list(zone, zone->nand_chip->max_erase_times);
    if(max_ec_blk == NULL)
    {
        return NFTL_FAILURE;    /* Generally not */
    }

    max_ec = max_ec_blk->erase_count;
    if(max_ec < (zone->nand_chip->max_erase_times/5))
    {
        return NFTL_FAILURE;   /* Free block is relatively new and should not be swapped in */
    }
    ...
    /** The higher the ec is used, the lower the threshold for static WL (the smaller the difference between max_ec and min_ec)
     *  When the use of ec is not too high, the threshold for static WL is relatively high to prevent static WL from being too frequent.
     */
    if (zone->nand_chip->max_erase_times * 2 < max_ec * 3) //Greater than 2000 erase_pan must be greater than 250
    {
        zone->s_wl.erase_span = zone->nand_chip->max_erase_times/12; //250
    }
    else if (zone->nand_chip->max_erase_times < max_ec * 2) //Greater than 1500 erase_pan must be greater than 500
    {
        zone->s_wl.erase_span = zone->nand_chip->max_erase_times/10; //300
    }
    else if(zone->nand_chip->max_erase_times < max_ec * 3)  //Greater than 1000 erase_pan must be greater than 1000
    {
    ...
    {
        return NFTL_FAILURE;   /* The difference does not meet the conditions */
    }
...
*Name         :
*Description  :
*Parameter    :The parameter invalid_page_data should be greater than 0
*Return       :
*Note         :
*****************************************************************************/
...
//            {
//                //Valid pages only need to read the spare area
