...
typedef struct __phy_block{
    ...
    uint16              info;                     //0xaa:Means there is page ECC error
    uint16              invalid_page_count;       //[0,pages_per_blk-1]
    uint16              erase_count;              //[0,10000]
    sint32              block_used_count;

    //A list of invalid pages in the block, a one-way list in the order of vaild_page_data size, used for garbage collection resident memory, which is restored at startup.
    struct __phy_block*    invalid_page_next;
    struct __phy_block*    invalid_page_prev;

    //Free block linked list, a one-way linked list in order of erase_data size
    struct __phy_block*    free_next;
    struct __phy_block*    free_prev;

    //Block uses a successively linked list
...
//This structure can only be added later, not modified

#define  SMART_VERSION     0xaaaaaa01   //Add members later without modifying the version number
...
typedef struct __smart{
    ...
    uint32  reserve[80-43];       //Fill in 320 bytes

}_smart;


//Resident memory, this data is written when this block is written to the last page.
...
//Schmidt oscillator
typedef struct{
    uint16   start_gc_free_blocks;  //free The block is less than this value to start gc
    uint16   stop_gc_free_blocks;   //free The block is greater than this value to stop the gc
    uint16   process;
...
typedef struct
{
    _phy_block_info  *              block_info;                    //Point to the currently used block
    _user_info                      user_info;                          //Points to the physical-to-logical mapping table of the currently used block
    uint16                          page_used;
...
struct _nftl_zone{
//  struct _nand_partition*         nand;                                       //The physical nand chip to which this physical partition belongs
    void*                           priv;
    struct  nand_chip_t*            nand_chip;                                //The physical nand chip to which this physical partition belongs
    char                            version[8];
    uchar                           zone_no;                                    //The number of this partition
    uchar                           zone_attr;                                  //Properties of this partition
    uint16                          blocks;                                     //How many physical blocks are available in this physical partition?
    uint16                          bad_block;
    uint16                          free_block_num;                             //Number of free blocks in this partition
    uint32                          logic_cap_in_sects;                         //The logical capacity of this physical partition
    uint32                          logic_cap_in_page;
    uint32                          backup_cap_in_sects;                        //This partition leaves enough backup space for the algorithm
    _nand_block                     zone_start_blk_NO;                          //The number of the first physical block
    _phy_block_info*                zone_start_phy_block;                       //Points to the first element of the physical block array
    _phy_block_info*                zone_end_phy_block;                         //Points to the first element of the physical block array

#if  MALLOC_FLAG
    _mapping_page*                  zone_logic_page_map[MAX_LOGIC_MAP_NUM];     //Point to the first element of the logical page mapping table
#else
    _mapping_page*                  zone_logic_page_map;                        //Point to the first element of the logical page mapping table
#endif

    _phy_block_info                 free_head;                                  //Point to the free block to arrange the linked list
    _phy_block_info                 invalid_page_head;                          //Point to the list of invalid pages in the block to arrange the list
    _phy_block_info                 block_used_head;                            //Point to the used block to arrange the linked list in order
    _user_info                      zone_phy_page_map_for_gc;                   //garbage_collect When using
    uchar*                          temp_page_buf;
