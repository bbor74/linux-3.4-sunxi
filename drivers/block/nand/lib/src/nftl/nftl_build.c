...
uint32 is_no_use_device(struct _nftl_zone * zone,uint32 size)
{
    ...
    {
        //This zone specifies the capacity
    ...
    for(zone->blocks=0; zone->blocks<total_blocks; p_phy_block_info++)
    {
        ...
        {
            //If it's not flash that hasn't been upgraded, the first good block should be the bad block mark.
...
uint32 build_zone_list_first(struct _nftl_zone * zone,uint32 size)
{
    ...
    //If it is the last zone, don't write down the end mark
    ...
    //On the next block, write down the zone end mark
...
uint32 build_zone_list(struct _nftl_zone * zone)
{
    ...
    //The second time you scan the block, only the blocks in the linked list used by the block are scanned.
...
uint32 first_scan_all_blocks(struct _nftl_zone * zone)
{
    ...
    //Scan the block for the first time
...
//      NFTL_ERR("[NE]block no: %d\n",p_phy_block_info->phy_block.Block_NO);
        ...
        if(zone->nftl_nand_is_blk_good(zone,&phy_op_par) == NFTL_YES) //Good block
        {
...
//          NFTL_ERR("[NE]block: %d page: [0x%2x],[0x%2x],[0x%2x],[0x%2x] erase:[0x%2x],[0x%2x]count:[0x%2x],[0x%2x],[0x%2x],[0x%2x]\n",phy_op_par.phy_page.Block_NO,spare_data[0],spare_data[1],spare_data[2],spare_data[3],spare_data[4],spare_data[5],spare_data[6],spare_data[7],spare_data[8],spare_data[9]);
            ...
            // 1:See if it's a special block, if it's a special block, jump out
            if(is_ftl_end_flag_page(spare_data) == NFTL_YES)
            {
                NFTL_ERR("[NE] ftl end block! %d!\n",p_phy_block_info->phy_block.Block_NO);
                break;
            }
            //2:If it is a block that has not been written, put it in the free list
            ...
            //3: Put this block into the linked list used by the block
            else if(((is_ftl_logic_page_data(spare_data) == NFTL_YES) || (is_function_info_page(spare_data) == NFTL_YES)) && ((ret == 0)||(ret == ECC_LIMIT)) )   //Is a valid logic page address
            {
                zone->blocks++;
            ...
            {
                //An undefined value appears in the spare area of page 0!！！
...
uint32 second_scan_all_blocks(struct _nftl_zone * zone)
{
    ...
    for(p_phy_block_info=zone->block_used_head.block_used_next; p_phy_block_info!=NULL; p_phy_block_info=p_phy_block_info->block_used_next)
    {
        ...
        if((is_phy_mapping_page(spare_data) == NFTL_YES) && ((ret == 0)||(ret == ECC_LIMIT)) )
        {
            ...
            p_phy_block_info->invalid_page_count++; //It has been confirmed that the last page stores the mapping table, then invalid_page_data plus 1
        ...
        {
            //The current used block of the zone.There are at most 2, and it is the last one in the linked list
            ...
            {
                //Found 3 current used blocks? Impossible!
        ...
        else
        {
            ...
            {
                //There are 2 current blocks in the crosstalk algorithm, and the last block used is the second one.
                //currentblock, for backup, if he has a problem, just delete it directly.
    ...
    //Scan the list of invalid pages used by the block, and establish a list of invalid pages in the block
    ...
    //Organize the list of invalid pages in the block, and put all the blocks with invalid pages into the free list
    ...
    if((p_phy_block_info_last != NULL)&&(current_block_first != NULL))
    {
        ...
        {//The used count of the current block should be the largest, and it is only 1.5 times larger than other blocks.
            NFTL_ERR("[ND]something strange %d,%d!!\n",p_phy_block_info_last->block_used_count,current_block_first->block_used_count);
        }
    }

    //Get zone_current_used_block
    zone->current_block.block_info = NULL;
...
uint32 recover_phy_page_mapping(struct _nftl_zone * zone,_phy_block_info* phy_block_ptr,uint32* data)
{
    ...
    for(i=0; i<zone->nand_chip->pages_per_blk; i++)
    {
        ...
        {
            //This page is not used, the last unused page has been reached
...
//          }
    ...
    if(i < (zone->nand_chip->pages_per_blk-2))  //Fill in an invalid data
    {
        if(!(zone->zone_attr&SUPPORT_COSS_TALK))
        {
        }
...
*Parameter    :
*Return       :
*Note         :There can be invalid pages, there can be no empty pages
*****************************************************************************/
uint32 recover_logic_page_mapping(struct _nftl_zone* zone,_phy_block_info* p_phy_block_info,uint32* data,uint32 phy_page_nums)
{
    ...
    for(i=0; i<phy_page_nums; i++)
    {
        ...
        if(logic_page < zone->logic_cap_in_page)
        {
            ...
            if(logic_page_map->Block_NO != 0xffff) //Has been assigned a value
            {
...
{
    NFTL_DBG("[ND] nand2.5 version:0001 date:%s %s\n",__DATE__,__TIME__);
