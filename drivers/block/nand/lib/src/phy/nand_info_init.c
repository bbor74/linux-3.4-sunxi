...
unsigned int calc_crc32(void * buffer, unsigned int length)
{
	...
	unsigned int CRC32 = 0xffffffff; //Set the initial value
	nand_crc32.CRC = 0;

	for( i = 0; i < 256; ++i)//Use ++i to improve efficiency
	{
		nand_crc32.CRC = i;
		for( j = 0; j < 8 ; ++j)
		{
			//This loop actually uses the "calculation method" to obtain the CRC check code
			if(nand_crc32.CRC & 1)
				nand_crc32.CRC = (nand_crc32.CRC >> 1) ^ 0xEDB88320;
			else //0xEDB88320 is the value of CRC-32 multiple expressions
				nand_crc32.CRC >>= 1;
		}
		nand_crc32.CRC_32_Tbl[i] = nand_crc32.CRC;
	}

	CRC32 = 0xffffffff; //Set the initial value
    for( i = 0; i < length; ++i)
    {
