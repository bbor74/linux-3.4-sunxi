...
__hdle h_debe0ahbclk,h_debe0dramclk;
__hdle h_debe1ahbclk,h_debe1dramclk;
__hdle h_defe0ahbclk,h_defe0dramclk;
__hdle h_defe1ahbclk,h_defe1dramclk;
...
__hdle h_debe0mclk,h_debe1mclk,h_defe0mclk,h_defe1mclk;
__hdle h_lcd0ch0mclk0,h_lcd0ch1mclk1;
__hdle h_lcd1ch0mclk0,h_lcd1ch1mclk1;
__hdle h_dsimclk_s,h_dsimclk_p;
__hdle h_hdmimclk,h_hdmimclk_ddc;
...
__s32 image_clk_exit(__u32 screen_id)
{
	if(screen_id == 0) {
		...
		g_clk_status &= (CLK_DEBE0_AHB_OFF  & CLK_DEBE0_DRAM_OFF);
#endif
		OSAL_CCMU_MclkOnOff(h_debe0mclk, CLK_OFF);
		...
		g_clk_status &= (CLK_DEBE1_AHB_OFF  & CLK_DEBE1_DRAM_OFF);
#endif
		OSAL_CCMU_MclkOnOff(h_debe1mclk, CLK_OFF);
...
__s32 scaler_clk_exit(__u32 scaler_id)
{
	if(scaler_id == 0) {
		...
		g_clk_status &= (CLK_DEFE0_AHB_OFF  & CLK_DEFE0_DRAM_OFF);
#endif
		OSAL_CCMU_MclkOnOff(h_defe0mclk, CLK_OFF);
		OSAL_CCMU_CloseMclk(h_defe0mclk);

		g_clk_status &= CLK_DEFE0_MOD_OFF;
	}	else if(scaler_id == 1) {
#ifdef RESET_OSAL
		...
		g_clk_status &= (CLK_DEFE1_AHB_OFF  & CLK_DEFE1_DRAM_OFF);
#endif
		OSAL_CCMU_MclkOnOff(h_defe1mclk, CLK_OFF);
		OSAL_CCMU_CloseMclk(h_defe1mclk);

		g_clk_status &= CLK_DEFE1_MOD_OFF;
	}
...
__s32 scaler_clk_on(__u32 scaler_id)
{
	if(scaler_id == 0) {
		...
		OSAL_CCMU_MclkOnOff(h_defe0ahbclk, CLK_ON);
		OSAL_CCMU_MclkOnOff(h_defe0dramclk, CLK_ON);
		g_clk_status |= ( CLK_DEFE0_DRAM_ON | CLK_DEFE0_AHB_ON);
#endif
		OSAL_CCMU_MclkOnOff(h_defe0mclk, CLK_ON);

		g_clk_status |= CLK_DEFE0_MOD_ON;
	}	else if(scaler_id == 1)	{
#ifdef RESET_OSAL
		OSAL_CCMU_MclkReset(h_defe1mclk, RST_INVAILD);
#endif
		...
		OSAL_CCMU_MclkOnOff(h_defe1ahbclk, CLK_ON);
		OSAL_CCMU_MclkOnOff(h_defe1dramclk, CLK_ON);
		g_clk_status |= (  CLK_DEFE1_DRAM_ON | CLK_DEFE1_AHB_ON);
#endif
		OSAL_CCMU_MclkOnOff(h_defe1mclk, CLK_ON);

		g_clk_status |=  CLK_DEFE1_MOD_ON ;
	}
	return	DIS_SUCCESS;
}
...
__s32 lcdc_clk_exit(__u32 screen_id)
{
	if(screen_id == 0) {
		...
		g_clk_status &= CLK_LCDC0_AHB_OFF ;
#endif
		OSAL_CCMU_MclkOnOff(h_lcd0ch0mclk0, CLK_OFF);
		OSAL_CCMU_MclkOnOff(h_lcd0ch1mclk1, CLK_OFF);
		...
		g_clk_status &= CLK_LCDC1_AHB_OFF ;
#endif
		OSAL_CCMU_MclkOnOff(h_lcd1ch0mclk0, CLK_OFF);
		OSAL_CCMU_MclkOnOff(h_lcd1ch1mclk1, CLK_OFF);
...
__s32 hdmi_clk_init(void)
{
	...
	g_clk_status |= CLK_HDMI_MOD_DDC_ON;
