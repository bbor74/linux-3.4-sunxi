...
#define FT5402_IC_INFO  "\n============================================================== \
                         ...
                         \nVERSION:2013-11-02_13:40\n"
                         

//extern int m_inet_ctpState;

//#define FIREWARE_DESTORY
...
//mbg ++ 20131209
static int ctp_ft5402_powersupplyall = 0;
...
static const int chip_id_value[] = {0x03,0xa3};
...
#define BUFFER_LEN (2)            //len == 2 
	...
	if ( 0xa6 == a || a != b ){
		...
		#ifdef FIREWARE_DESTORY
		else if((chip_id == 0x03)||(chip_id == 0xa3))
		#else
		...
		#endif
		{
			...
			if ( 0xa6 == a || a <= 0x18 )
			{
				pbt_buf_5402= CTPM_FW;
				i_ret =  ft5402_ctpm_fw_upgrade(client,pbt_buf_5402,sizeof(CTPM_FW));
				if (i_ret != 0){
					printk("[FTS] upgrade failed i_ret = %d.\n", i_ret);
				} 
				else {
...
static void ft5x_resume_events (struct work_struct *work)
{
...
#endif
	msleep(100);
	if(chip_id == 0x3) //chip_type = ft5402 add 2012.11.20 
	{
		msleep(350);
	...
	if(chip_id == 0x03 ){
		...
		if (ft5402_Get_Param_From_Ini(FT5402_CONFIG_NAME) >= 0)
			...
			printk("Get ft5402 param from INI file failed\n");
#else
		msleep(200);
		while(i<5){
			dprintk(DEBUG_INIT,"-----------------------------------------Init ic param\r\n");
			if (ft5402_Init_IC_Param(this_client) >=0 ){
				dprintk(DEBUG_INIT,"---------------------------------------get ic param\r\n");
				if(ft5402_get_ic_param(this_client) >=0)
					break;
			}
			i++;
		}
...
static void ft5x_init_events (struct work_struct *work)
{
	...
	fts_ctpm_fw_upgrade_with_i_file(this_client);
#endif
...
static int ft5x_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	ft5x_ts->early_suspend.level = EARLY_SUSPEND_LEVEL_DISABLE_FB + 1 ;//EARLY_SUSPEND_LEVEL_BLANK_SCREEN + 1;
...
{  
	...
	if(SCIRPT_ITEM_VALUE_TYPE_INT != script_get_item("ctp_para", "ctp_ft5402_powersupplyall", &val))
	{
		ctp_ft5402_powersupplyall=0;
	}
	else
	{
		ctp_ft5402_powersupplyall = val.val;
	}

	printk("ctp_ft5402_powersupplyall = %d\n",ctp_ft5402_powersupplyall);
